...
import { StrictToolManager } from './tools-accesslint/strictToolManager';
...
export class LLMToolCallParser {
  private toolDefinitions: Map<string, ToolDefinition> = new Map();
  private strictToolManager?: StrictToolManager;
  private mistakeCounter: number = 0;
  private readonly maxMistakes: number = 3;
  private streamingBuffer: string = '';

  constructor(tools: ToolDefinition[], strictToolManager?: StrictToolManager) {
    tools.forEach(tool => {
   ...
   */
  private parseStrictXML(response: string): ParseResult {
    ...
    const xmlMatches = [...response.matchAll(/<(\w+)>([\s\S]*?)<\/\1>/g)];
      ...
      if (!this.toolDefinitions.has(toolName)) {
        this.incrementMistakeCounter();
        throw new Error(this.createStrictError('UNKNOWN_TOOL', `Unknown tool: ${toolName}`, 
          `Available tools: ${Array.from(this.toolDefinitions.keys()).join(', ')}`));
      }
      ...
      try {
        ...
        throw new Error(this.createStrictError('MALFORMED_XML', 
          `Failed to parse tool ${toolName}: ${errorMessage}`));
      }
    }
   ...
   */
  private hasXMLBlocks(response: string): boolean {
   ...
   */
  private createStrictError(type: 'MALFORMED_XML' | 'UNKNOWN_TOOL' | 'SCHEMA_VALIDATION' | 'NO_XML_BLOCKS' | 'MAX_MISTAKES', message: string, details?: string): string {
...
✅ ALWAYS use exact parameter names as shown above`;
   ...
   */
  private getToolSchema(toolName: string): string {
...
Required parameters: ${required.join(', ')}`;
   ...
   */
  private parseXmlParameters(content: string): any {
    const params: any = {};
    ...
    while ((match = paramPattern.exec(content)) !== null) {
      ...
      let paramValue: any = match[2].trim();
   ...
   */
  validateToolCall(toolCall: ParsedToolCall): { valid: boolean; errors: string[] } {
