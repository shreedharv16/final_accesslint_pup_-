...
import { DiffApprovalRequest, DiffApprovalResponse, DiffViewerManager } from './DiffViewerManager';
...
export class DiffViewerProvider {
  ...
  constructor(
    ...
    private diffManager: DiffViewerManager
  ) {}
   ...
   */
  private handleApprovalResponse(response: DiffApprovalResponse): void {
   ...
   */
  private handlePreviewRequest(requestId: string, approvedHunks: string[]): void {
  ...
  private getWebviewContent(): string {
    ...
    const cssUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this.context.extensionUri, 'webviews', 'diffViewer.css')
    );
    const jsUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this.context.extensionUri, 'webviews', 'diffViewer.js')
    );
    
    // Load highlight.js for syntax highlighting
    const highlightJsUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this.context.extensionUri, 'node_modules', 'highlight.js', 'lib', 'highlight.min.js')
    );
    const highlightCssUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this.context.extensionUri, 'node_modules', 'highlight.js', 'styles', 'vs2015.min.css')
    );
