...
exports.generateEnhancedAccessibilityPrompt = exports.getPatternRecommendations = exports.ACCESSIBILITY_PATTERNS = void 0;
/**
 * Comprehensive list of accessibility patterns that agents should detect and fix
 */
exports.ACCESSIBILITY_PATTERNS = [
    // ============================================
 ...
 */
function getPatternRecommendations(issues) {
    let recommendations = `
...
`;
    ...
    const relevantPatterns = exports.ACCESSIBILITY_PATTERNS.filter(pattern => {
        const issueText = issues.join(' ').toLowerCase();
        const patternKey = pattern.name.toLowerCase();
        // Match patterns to issues
        if (patternKey.includes('alt') && issueText.includes('image'))
            return true;
        if (patternKey.includes('label') && issueText.includes('label'))
            return true;
        if (patternKey.includes('form') && issueText.includes('form'))
            return true;
        if (patternKey.includes('landmark') && issueText.includes('landmark'))
            return true;
        if (patternKey.includes('button') && issueText.includes('button'))
            return true;
        if (patternKey.includes('clickable') && issueText.includes('click'))
            return true;
        return false;
    });
...
`;
        ...
        exports.ACCESSIBILITY_PATTERNS.forEach(pattern => {
            recommendations += `
...
exports.getPatternRecommendations = getPatternRecommendations;
/**
 * Generate a comprehensive accessibility fix prompt with pattern intelligence
 */
function generateEnhancedAccessibilityPrompt(issues, context = {}) {
...
exports.generateEnhancedAccessibilityPrompt = generateEnhancedAccessibilityPrompt;
