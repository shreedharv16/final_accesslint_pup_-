...
- list_directory with recursive true - get full tree structure`,
    ...
    async listDirectory(dirPath, includeHidden, fileExtensions) {
        const entries = [];
        try {
    ...
    async listRecursive(dirPath, maxDepth, includeHidden, fileExtensions, currentDepth = 0) {
        ...
        try {
            ...
            for (const entry of currentEntries) {
                if (entry.type === 'directory') {
                    ...
                    const subEntries = await this.listRecursive(fullSubPath, maxDepth, includeHidden, fileExtensions, currentDepth + 1);
    ...
    logExecution(input, result) {
            ...
            this.context.webviewProvider.postMessage({
                ...
                result: {
                    ...
                    entries: result.entries && result.entries.length > 50
                        ? result.entries.slice(0, 50).concat([{ name: '... (truncated)', type: 'file', path: '...' }])
                        : result.entries
                },
                timestamp: new Date()
            });
        ...
        if (this.context.outputChannel) {
            ...
            this.context.outputChannel.appendLine(`[LIST_DIR ${status}] ${input.path} - Found ${result.totalFiles || 0} files, ${result.totalDirectories || 0} directories`);
            if (result.error) {
