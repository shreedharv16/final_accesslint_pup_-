...
- Use this tool when you need to find files containing specific patterns`,
    ...
    async searchDirectory(dirPath, regex, matches, includePatterns, excludePatterns, maxResults) {
    ...
    async searchFile(filePath, regex, matches, maxResults) {
        if (matches.length >= maxResults)
            return;
    ...
    logExecution(input, result) {
        ...
        if (this.context.outputChannel) {
            ...
            this.context.outputChannel.appendLine(`[GREP_TOOL ${status}] Pattern: "${input.pattern}" - ${summary}`);
        }
    }
