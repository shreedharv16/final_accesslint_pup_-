...
exports.TestingAgentOrchestrator = void 0;
const vscode = __importStar(require("vscode"));
const agentSystemPrompt_1 = require("./agentSystemPrompt");
const todoListManager_1 = require("./todoListManager");
const fileContextTracker_1 = require("./fileContextTracker");
/**
 * TESTING-SPECIFIC ORCHESTRATOR
 * This is a specialized copy of AgentLLMOrchestrator for the Testing Menu.
 * It includes additional null-safety checks to prevent crashes on timeout/stop.
...
class TestingAgentOrchestrator {
    constructor(aiProviderManager, toolManager) {
        ...
        this.LOOP_DETECTION_WINDOW = 10 * 60 * 1000; // 10 minutes (increased from 5)
        this.MAX_SAME_TOOL_CALLS = 15; // Max same tool calls in window (increased to handle large batches)
        this.MAX_IDENTICAL_CALLS = 4; // Max identical calls (increased from 2)
        ...
        this.fileContextTracker = new fileContextTracker_1.FileContextTracker(workspaceRoot);
     ...
     */
    async startSession(goal, provider = 'anthropic') {
...
Please start by exploring the workspace structure using list_directory with path="." to understand the project structure, then implement what I've requested by creating or modifying the necessary files.`;
    ...
    detectInfiniteLoop(toolCalls) {
        ...
        for (const toolCall of toolCalls) {
            ...
            // Clean old entries
            this.toolCallHistory = this.toolCallHistory.filter(entry => currentTime - entry.timestamp < this.LOOP_DETECTION_WINDOW);
            ...
            if (toolCall.name === 'write_file') {
                ...
                const recentWriteCalls = this.toolCallHistory.filter(entry => entry.tool === 'write_file' &&
            ...
            const identicalCalls = this.toolCallHistory.filter(entry => entry.tool === toolCall.name && entry.input === JSON.stringify(toolCall.input));
            ...
            const recentIdentical = identicalCalls.filter(entry => currentTime - entry.timestamp < this.RAPID_CALL_WINDOW);
     ...
     */
    isValidExplorationPattern(toolCall, history) {
        const recentHistory = history.filter(entry => Date.now() - entry.timestamp < 2 * 60 * 1000 // Last 2 minutes
        );
...
Please provide a final answer based on the information you have already gathered from previous tool executions. Do not repeat the same tool calls.`
                    ...
                    if (isGPT5 && this.currentSession.iterations >= 5) { // Reduced from 8 to 5
                        const recentReadFiles = this.toolCallHistory.filter(entry => entry.tool === 'read_file' &&
                            (Date.now() - entry.timestamp) < 300000 // last 5 minutes
                        ).length;
                        const recentImplementations = this.toolCallHistory.filter(entry => (entry.tool === 'write_file' || entry.tool === 'edit_file') &&
                            (Date.now() - entry.timestamp) < 300000 // last 5 minutes
                        ).length;
                        ...
                        const shouldIntervene = (recentReadFiles >= 2 && !hasCurrentImplementationCall && explorationToImplementationRatio > 2) || (recentReadFiles >= 3 && explorationToImplementationRatio > 1.5) || (recentReadFiles >= 4 // Force intervention after 4 reads regardless
                        );
                        if (shouldIntervene) {
...
Example: If changing background to pink, edit the CSS/style files you've already read.`
                    ...
                    const toolResultsText = toolResults.map((result, index) => {
                        ...
                        const anthropicProvider = this.aiProviderManager.anthropicProvider;
                        if (anthropicProvider && typeof anthropicProvider.formatToolResult === 'function') {
                            return anthropicProvider.formatToolResult(toolCall.name, result, result.success);
                        }
    ...
    async applyContextManagement(messages) {
        ...
        if (anthropicProvider && typeof anthropicProvider.getContextManager === 'function') {
            ...
            if (result.wasModified) {
                this.outputChannel.appendLine(`🗜️ Context optimized: ${result.stats.truncatedMessages} messages removed, ${result.stats.tokensSaved} tokens saved`);
     ...
     */
    async executeToolCallsEfficiently(toolCalls) {
        const results = [];
        // Group tools by type for intelligent execution
        const readOnlyTools = toolCalls.filter(tc => ['read_file', 'list_directory', 'grep_search'].includes(tc.name));
        const writeTools = toolCalls.filter(tc => ['write_file', 'edit_file'].includes(tc.name));
        const completionTools = toolCalls.filter(tc => tc.name === 'attempt_completion');
        const otherTools = toolCalls.filter(tc => !['read_file', 'list_directory', 'grep_search', 'write_file', 'edit_file', 'attempt_completion'].includes(tc.name));
        ...
        for (const toolCall of toolCalls) {
            try {
                ...
                if (['write_file', 'edit_file'].includes(toolCall.name) &&
                    ...
                    const toolResult = await this.toolManager.executeToolWithConditionalApproval(toolCall.name, toolCall.input, needsApproval);
                    result = {
                    ...
                    result = await this.toolManager.executeTool(toolCall.name, toolCall.input, 'agent', // Use agent mode for proper logging
                ...
                if (result.success) {
                    ...
                    if (toolCall.name === 'read_file' && result.output) {
                        ...
                        if (filePath) {
                            this.fileContextTracker.cacheFileContent(filePath, result.output, toolCall.input?.limit, toolCall.input?.offset);
                        }
                    }
                    if (result.output) {
    ...
    async executeToolCallsParallel(toolCalls) {
        ...
        for (const toolCall of toolCalls) {
            try {
                ...
                const startTime = Date.now();
                const result = await this.toolManager.executeTool(toolCall.name, toolCall.input, 'agent' // Use agent mode for proper logging
                );
                const duration = Date.now() - startTime;
                // Validate result object
                if (!result) {
     ...
     */
    formatResponseToolAlreadyUsed(toolName) {
     ...
     */
    shouldRequestApproval(toolCall) {
     ...
     */
    bashCommandNeedsApproval(command) {
        if (!command)
            return true;
    ...
    async requestApproval(toolCall) {
        ...
        const choice = await vscode.window.showWarningMessage(message, { modal: true }, 'Approve', 'Deny', 'View Details');
     ...
     */
    shouldTriggerCompletion(toolResults, toolCalls) {
        if (!this.currentSession)
            return false;
        ...
        const hasReadFiles = toolResults.some(result => result.success &&
            result.output &&
            result.output.length > 100);
        // Check if we've explored the project structure
        const hasExploredStructure = this.currentSession.messages.some(msg => msg.toolResults?.some(tr => tr.result?.success && tr.result?.output?.includes('directory')));
        // Count successful tool executions with meaningful results
        const successfulToolExecutions = this.currentSession.messages.filter(msg => msg.toolResults?.some(tr => tr.result?.success && tr.result?.output && tr.result?.output.length > 50)).length;
        // Check if we're making progress or stuck
        const recentFileReads = this.toolCallHistory.filter(entry => entry.tool === 'read_file' &&
        ...
        if (isGPT5) {
            ...
            const hasExploredSufficiently = this.toolCallHistory.filter(entry => (entry.tool === 'list_directory' || entry.tool === 'read_file') &&
                (Date.now() - entry.timestamp) < 300000 // last 5 minutes
            ).length >= 2;
    ...
    manageContextBeforeCall() {
        ...
        if (anthropicProvider && typeof anthropicProvider.getTokenTracker === 'function') {
            ...
            if (currentTokens > 4000) { // Increased threshold to be less aggressive
                ...
                const toolResultMessages = this.currentSession.messages.filter(msg => msg.toolResults && msg.toolResults.length > 0).slice(-2); // Keep last 2 tool result messages
    ...
    createSystemPrompt() {
        const workspaceRoot = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath || '';
        return (0, agentSystemPrompt_1.createAgentSystemPrompt)(workspaceRoot, process.platform);
    }
    /**
     ...
     */
    notifyTodoListCreated(todoList) {
     ...
     */
    attemptToolCallFix(toolCall, errors) {
     ...
     */
    applyFileContextOptimization(toolCalls) {
     ...
     */
    manageGPT5ContextEfficiently() {
        if (!this.currentSession || this.config.provider !== 'openai')
            return;
        ...
        if (estimatedTokens > maxAllowedTokens) {
            ...
            const implementationMessages = messages.filter(msg => msg.content.includes('edit_file') ||
                msg.content.includes('write_file') ||
                msg.content.includes('CACHE HIT') ||
                msg.content.includes('INTERVENTION') ||
                msg.toolResults?.some(tr => tr.result?.success && ['write_file', 'edit_file'].includes(tr.result?.metadata?.toolName))).slice(-1); // Keep last 1 implementation message (reduced from 2)
    ...
    manageGPT5Context() {
        ...
        if (estimatedTokens > maxAllowedTokens) {
            ...
            const implementationMessages = messages.filter(msg => msg.content.includes('write_file') ||
                msg.content.includes('edit_file') ||
                msg.toolResults?.some(tr => tr.result?.success && tr.result?.metadata?.toolName === 'write_file')).slice(-2); // Keep last 2 implementation messages
            // Smart combination avoiding duplicates
            const preservedMessages = [];
            if (systemMessage)
                preservedMessages.push(systemMessage);
     ...
     */
    async executeToolBatch(toolCalls, batchType) {
        const results = [];
        if (batchType === 'read-only') {
            ...
            const promises = toolCalls.map(async (toolCall) => {
                try {
                    ...
                    if (toolCall._syntheticResult) {
                        const syntheticResult = toolCall._syntheticResult;
                        const duration = Date.now() - startTime;
...
exports.TestingAgentOrchestrator = TestingAgentOrchestrator;
//# sourceMappingURL=testingAgentOrchestrator.js.map