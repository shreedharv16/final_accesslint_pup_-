...
class ToolManager {
     ...
     * Execute a tool
     */
    async executeTool(toolName, input, mode = this.currentMode) {
        ...
        try {
            (0, loggingService_1.logDebug)(`🔧 Executing tool: ${toolName}`, this.context.userId, this.context.sessionId, 'agent', { input });
            let result;
            switch (toolName) {
                case 'read_file':
                ...
                case 'write_file':
                    ...
                    break;
                case 'edit_file':
                    result = await this.editFile(input.file_path, input.old_string, input.new_string, input.replace_all);
                    break;
                case 'grep_search':
            ...
            (0, loggingService_1.logInfo)(`✅ Tool executed: ${toolName} (${duration}ms)`, this.context.userId, this.context.sessionId, 'agent');
            return {
                ...result,
                metadata: {
                    toolName,
     ...
     */
    async grepSearch(pattern, path, fileType) {
        if (!this.context.workspaceFiles) {
            return {
                success: false,
                error: 'Workspace files not available'
